...
export async function getUltimosPrecios(marca: Marca) {
  // Obtener la última fecha disponible para la marca
  const { data: ultimaFechaData } = await supabase
    .from('fact_precios')
    .select(`
      dim_fecha!inner(fecha),
      dim_producto!inner(marca)
    `)
    .eq('dim_producto.marca', marca)
    .order('dim_fecha(fecha)', { ascending: false })
    .limit(1)

  if (!ultimaFechaData || ultimaFechaData.length === 0) return []

  const ultimaFecha = (ultimaFechaData[0].dim_fecha as any).fecha

  const { data } = await supabase
    .from('fact_precios')
    .select(`
      precio,
      dim_producto!inner(id, nombre, marca),
      dim_supermercado!inner(nombre),
      dim_fecha!inner(fecha)
    `)
    .eq('dim_producto.marca', marca)
